  ...
  <section>
    ...
    >
      <div class="user-item input-border"   v-if="info.email&&lic">
        <label class="fz14 c9" style="width:100px">已绑定邮箱号:</label>
      ...
      <el-form-item prop="email" class="el-from-ml100 user-item input-border" style="margin-bottom: 22px;" v-if="!info.email||!lic">
        <label class="label">新手机账号：</label>
        <el-input v-model="ruleForm.email" placeholder="手机号"><template slot="prepend" style="width:39px">+86</template></el-input>
      </el-form-item>
      <el-form-item prop="emailcode" class="user-item flex ctn" style="margin-left:-135px">
        <label class="label" v-if="info.email&&lic">邮箱验证码：</label>
        <label class="label" style="line-height:40px" v-if="!info.email||!lic">短信验证码：</label>
        <div class="Message-verify flex" v-if="info.email&&lic">
          <el-input placeholder="邮箱验证码" v-model="ruleForm.emailcode" clearable></el-input>
          <el-button
        ...
        <div class="Message-verify flex" v-if="!info.email||!lic">
          ...
          <el-button
            class="btn-verify btn-verify-span"
            @click="codeNewFlag?()=>{}:Getvalidations()"
            style="cursor:pointer;"
            v-if="!lic"
  ...
  async  Getvalidations(){
    //   alert(this.lic)
        if(this.info.phoneNum&&this.lic){
          if(this.codeFlag)return
                this.ruleForm.email = this.info.phoneNum
                this.vaildName=this.ruleForm.email
                let Getvalidation = await this.$axios.post("/send/sendSms", {
                phoneNumList: [this.ruleForm.email],
                sendType: 1
                });
                if (Getvalidation.code == 0) {
                    if(this.lic){
